        ...
        TOOL_GUIDE = (
            "## Tool Selection\n"
              ...
              #  "- Never perform actions outside defined tool contracts.\n"
                ...
                "- When ambiguous (e.g., duplicate line items), ask for short confirmation before acting.\n"
                "- Only proceed once the user replies with 'yes' or clear intent.\n"
                ...
                "- Quantity and unit price → `quantity`, `unit_cost`. Never send nulls.\n"
                "- Write responses in 2–5 clear bullets, no code blocks. Use US currency style.\n"
                "- For summaries, include subtotals, dates, statuses clearly.\n"

                "## Error Handling\n"
                "- Retry once if a record is not found. Otherwise, list and ask user to reselect.\n"
                "- If a tool doesn't exist, say so briefly and offer a closest match if helpful.\n"

                "## Current Order Context\n"
                f"- Default order_id context: {req.order_id or '(none)'}"
                )
